...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = await axios.post('http://localhost:3000/api/auth/register', {name:name, email, password});
    ...
    <div className="min-h-screen flex items-center justify-center bg-black py-12 px-4 sm:px-6 lg:px-8">
      ...
      <Card className="w-full max-w-md relative z-10 glass border-gray-600 bg-black text-white ">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl font-bold text-center">Register to Noyeah</CardTitle>
        ...
        <CardFooter className="flex flex-col space-y-4">
          <div className="text-center text-sm text-gray-400">
            ...
            <Link href="/login" className="text-blue-400 hover:text-purple-300 font-medium">
              Login
            </Link>
          </div>
        </CardFooter>
